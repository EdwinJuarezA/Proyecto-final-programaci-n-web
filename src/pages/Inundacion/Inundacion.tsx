...
import { eyeOutline } from 'ionicons/icons'; // Importar el ícono correctamente

const Inundacion: React.FC = () => {
    const [notes, setNotes] = useState<NoteType[]>([]);
    const [showModal, setShowModal] = useState(false);
    const [selectedNote, setSelectedNote] = useState<NoteType | null>(null);

    useEffect(() => {
        axios.get('/notesInundaciones')
            .then(response => {
                console.log('Datos recibidos:', response.data); // Añadir log para verificar datos
        ...
        <IonPage>
            <IonHeader>
                <IonToolbar>
                    ...
                    <IonTitle>Inundaciones</IonTitle>
                </IonToolbar>
            </IonHeader>
            <IonContent>
                {notes.map(note => (
                ...
                <IonModal isOpen={showModal} onDidDismiss={() => setShowModal(false)}>
                    <IonHeader>
                        <IonToolbar>
                            <IonTitle>Detalle de la Inundacion</IonTitle>
